...
#
# Stages:
#   1. (assumes) manifests/all_genomes.tsv + manifests/refseq_urls.tsv exist
...
#   5. Concatenate per-chunk TSVs into the canonical results/all_*.tsv
...
if [ ! -s results/_chunks/tara_loci.tsv ]; then
    awk -F'\t' 'NR==1 || $3=="tara"' manifests/all_genomes.tsv > "$LOGDIR/tara_manifest.tsv"
    "$PYTHON" scripts/scan_telotrons.py \
        --manifest "$LOGDIR/tara_manifest.tsv" \
        --canonical-motifs manifests/canonical_motifs.tsv \
        --refseq-dir raw/refseq --tara-dir raw/tara \
        --motifs "$MOTIFS" \
        --threads "$SCAN_THREADS" \
        --loci results/_chunks/tara_loci.tsv \
        --introns results/_chunks/tara_introns.tsv \
        --summary results/_chunks/tara_summary.tsv \
...
awk -F'\t' 'NR>1 && $3!="tara"' manifests/all_genomes.tsv | sort -k1,1 > "$LOGDIR/refseq_manifest_rows.tsv"
sort -k1,1 manifests/refseq_urls.tsv > "$LOGDIR/refseq_urls_sorted.tsv"
...
for chunk_url_file in "$LOGDIR"/chunk_url_*; do
    ...
    LOCI_OUT="results/_chunks/refseq_loci_${CHUNK_ID}.tsv"
    INTRONS_OUT="results/_chunks/refseq_introns_${CHUNK_ID}.tsv"
    SUMMARY_OUT="results/_chunks/refseq_summary_${CHUNK_ID}.tsv"
    ...
    head -1 manifests/all_genomes.tsv > "$LOGDIR/chunk_manifest.tsv"
...
concat results/all_telotron_loci.tsv \
       results/_chunks/tara_loci.tsv results/_chunks/refseq_loci_*.tsv
concat results/all_introns_scanned.tsv \
       results/_chunks/tara_introns.tsv results/_chunks/refseq_introns_*.tsv
concat results/all_species_raw_summary.tsv \
       results/_chunks/tara_summary.tsv results/_chunks/refseq_summary_*.tsv
...
"$PYTHON" scripts/filter_final_set.py \
    ...
    --negatives results/final_negative_controls.tsv \
    2>&1 | tee -a "$LOGDIR/filter.log"

"$PYTHON" scripts/dedup_telotrons.py \
    --final results/final_telotron_set.tsv \
    --refseq-dir raw/refseq --tara-dir raw/tara \
    --out-final results/final_telotron_set_dedup.tsv \
    --out-log results/dedup_log.tsv \
    --threads "$SCAN_THREADS" \
    2>&1 | tee -a "$LOGDIR/dedup.log"

# Replace working file with dedup set; preserve pre-dedup for provenance.
mv results/final_telotron_set.tsv results/final_telotron_set.prededup.tsv
cp results/final_telotron_set_dedup.tsv results/final_telotron_set.tsv

"$PYTHON" scripts/analyze_telotrons.py \
    --final results/final_telotron_set.tsv \
    --introns results/all_introns_scanned.tsv \
    --species results/final_species_summary.tsv \
    --boundary-kmers results/boundary_kmer_enrichment.tsv \
    --distance results/distance_to_end.tsv \
    --architecture results/architecture_summary.tsv \
    --threads "$SCAN_THREADS" \
...
awk -F'\t' 'NR>1 && $12+0 > 0 {print $1}' results/final_species_summary.tsv > "$LOGDIR/positive_ids.txt"
...
"$PYTHON" scripts/classify_telotron_architecture.py \
    --final results/final_telotron_set.tsv \
    --refseq-dir raw/refseq --tara-dir raw/tara \
    --out-loci results/final_telotron_set_architecture.tsv \
    --out-kmers results/boundary_kmers_by_architecture.tsv \
...
"$PYTHON" scripts/blast_linkers.py \
    --arch-tsv results/final_telotron_set_architecture.tsv \
    --refseq-dir raw/refseq --tara-dir raw/tara \
    --workdir results/blast_linkers \
    --out-own results/linker_blast_hits_own_genome.tsv \
    --out-all results/linker_blast_hits_all_genomes.tsv \
    --threads "$SCAN_THREADS" \
...
"$PYTHON" scripts/find_interstitial_arrays.py \
    ...
    --refseq-dir raw/refseq --tara-dir raw/tara \
    --motifs "$MOTIFS" \
    --out results/interstitial_arrays.tsv \
    --threads "$SCAN_THREADS" \
...
"$PYTHON" scripts/plot_telotrons.py \
    ...
    --distance results/distance_to_end.tsv \
    --architecture results/architecture_summary.tsv \
    --outdir results/figures 2>&1 | tee -a "$LOGDIR/plot_telotrons.log"
...
"$PYTHON" scripts/plot_pipeline_stages.py \
    --manifests manifests/all_genomes.tsv \
    --raw-summary results/all_species_raw_summary.tsv \
    --species-final results/final_species_summary.tsv \
    --distance results/distance_to_end.tsv \
    --architecture-loci results/final_telotron_set_architecture.tsv \
...
log "=== ALL DONE ==="
log "results/ size: $(du -sh results | awk '{print $1}')"
