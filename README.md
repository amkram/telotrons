...
| assembly_urls | Derive per-genome FNA + GFF download URLs from every assembly's FTP path | refseq_euk.tsv + genbank_euk.tsv | work/manifests/assembly_urls.tsv | n/a |
...
| download_assemblies | Fan out per-genome curls (`xargs -P 8`); RefSeq → data/raw/refseq/, GenBank → data/raw/genbank/ | work/manifests/assembly_urls.tsv | data/raw/refseq/.done + data/raw/genbank/.done | n/a |
| canonical_motifs | Emit curated per-genome/per-group telomere-motif TSV so scan uses literature motif over contig-end inference | work/manifests/all_genomes.tsv + config `canonical_telomere_motifs` | work/manifests/canonical_motifs.tsv | n/a |

## 2. Core survey → confident set

| Rule | Purpose | Key inputs | Key outputs | Script |
|---|---|---|---|---|
| scan_all | Per-intron motif coverage, splice signals, orientation, distance-to-end for every RefSeq + GenBank + Tara genome | all_genomes.tsv + refseq/genbank/tara FASTAs+GFFs | all_telotron_loci.tsv + all_introns_scanned.tsv | scripts/scan_telotrons.py |
...
| dedup_telotrons | Collapse doubly-assembled exons / close paralogs by within-species flank blastn union-find; keep longest-intron representative | final_telotron_set.tsv + refseq/genbank/tara FASTAs | final_telotron_set_dedup.tsv + dedup_log.tsv | scripts/dedup_telotrons.py |
...
| Rule | Purpose | Key inputs | Key outputs | Script |
|---|---|---|---|---|
| extract_telotron_fasta | Per-species×architecture FASTAs + flanked [LEFT100][INTRON][RIGHT100] text extracts | final_telotron_set_architecture.tsv + refseq/genbank/tara FASTAs | work/results/telotron_fasta/ + telotron_flanked/ (sentinel) | scripts/extract_telotron_fasta.py |
...
| extract_non_telotron_fasta | Same FASTA + flanked extraction for the control set (reuses telotron extractor) | non_telotron_controls.tsv + refseq/genbank/tara FASTAs | work/results/non_telotron_fasta/ (sentinel) | scripts/extract_telotron_fasta.py |

## 4. Interstitial contrast

| Rule | Purpose | Key inputs | Key outputs | Script |
|---|---|---|---|---|
| find_interstitial_arrays | Locate non-terminal, non-genic, non-intronic telomeric arrays; 6-frame ORF mask (was a separate rule) built inline | all_species_raw_summary.tsv + refseq/genbank/tara FASTAs | work/results/interstitial_arrays.tsv | scripts/make_unannotated_mask.py + scripts/find_interstitial_arrays.py |
...
| find_tert | Miniprot + Pfam-architecture TERT search (RBD upstream of RT), iterative re-seeding — HMM-based (replaces BLAST) | tert seeds + HMMs + refseq/genbank/tara FASTAs | tert_deep_homology/confirmed_tert.tsv + all_confirmed.faa | scripts/find_tert_deep_homology.py |

## 6. Telotron-gene orthologs

| Rule | Purpose | Key inputs | Key outputs | Script |
|---|---|---|---|---|
| telotron_ortholog_align | Per telotron-host gene, miniprot-map ortholog in each panel genome, protein-align it, then DNA-align telotron vs orthologous intron to resolve fill-vs-create | final_telotron_set_architecture.tsv + refseq/genbank/tara FASTAs | work/results/telotron_orthologs/ (sentinel) | scripts/telotron_ortholog_align.py |
| plot_telotron_ortholog_loci | Compiled per-locus PDF: flanking-exon protein MSA + intron DNA alignment | telotron_orthologs sentinel | work/results/figures/telotron_ortholog_loci.pdf | scripts/plot_telotron_ortholog_loci.py |
...
| Rule | Purpose | Key inputs | Key outputs | Script |
|---|---|---|---|---|
| telotron_gene_class | Host-gene-class dissection (host vs disjoint non-host intron count + gene length; per-intron rate vs gene properties + 5' preference). Iterates the confident-species set dynamically | ARCH_TSV + confident_species.tsv + refseq/genbank GFFs | work/results/figures/telotron_gene_class.png | scripts/telotron_gene_class.py |
...
| nucleosome_features | One rule: build insertion-site + flank FASTAs, then composition / 10-bp WW periodicity / CpG panel (telomere-masked flanks, BH-FDR). NuPoP occupancy arm retired 2026-07-14 | ARCH_TSV + non_telotron_controls.tsv + refseq/genbank/tara FASTAs | work/results/nucleosome/nucleosome_feature_summary.png | scripts/nucleosome_inputs.py + scripts/nucleosome_control_inputs.py + scripts/nucleosome_features.py |
| nucleosome_withingene | Within-gene control: telotron introns vs same-gene sibling introns vs random non-host introns (separates gene-class confound from local targeting) | ARCH_TSV + non_telotron_controls.tsv + refseq/genbank/tara FASTAs | work/results/nucleosome/withingene_control.txt | scripts/nucleosome_withingene.py |
